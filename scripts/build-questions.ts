 ...
 *
 * Also normalises the text, mirrors every referenced image locally, emits the 7 translation
...
async function main(): Promise<void> {
  ...
  for (const lang of LANGUAGES) {
    const bundle = bundles.get(lang);
    if (!bundle) fail(`missing bundle for ${lang}`);
    await writeFile(path.join(I18N_DIR, `questions.${lang}.json`), stableStringify(bundle), 'utf8');
  }
